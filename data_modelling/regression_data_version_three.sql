...
database demo_db;
CREATE
OR REPLACE TABLE regression_data_version_three AS (
    WITH sama_2015 AS
             (
                 SELECT facility_id,
                        MIN(distance) AS min_distance_sama_2015
                 FROM conflict_facility_fact_v3
                   WHERE year = 2015
AND conflict_type='sama'
    ...
    WITH sama_2016 AS
             (
                 SELECT facility_id,
                        MIN(distance) AS min_distance_sama_2016
                 FROM conflict_facility_fact_v3
                   WHERE year = 2016
AND conflict_type='sama'
